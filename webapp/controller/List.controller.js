sap.ui.define([
    ...
    return BaseController.extend("project3.controller.List", {
        ...
        onSearch: function (oEvent) {
            ...
            if (sQuery) {
                this._oListFilterState.aSearch = [new Filter("ProjectId", FilterOperator.Contains, sQuery)];
            } else {
        ...
        onOpenViewSettings: function (oEvent) {
            ...
            if (!this.byId("viewSettingsDialog")) {
                Fragment.load({
                    ...
                    name: "project3.view.ViewSettingsDialog",
                    controller: this
        ...
        openSorterDialog: function () {
            if (!this.vDialog) {
                this.vDialog = this.loadFragment({
                    name: "project3.view.fragments.Sorter"
                });
            }

            this.vDialog.then(function (vDialog) {
                vDialog.open();
            });
        },

        handleConfirmSorter: function (oEvent) {

            let items = this.byId('list');

            let key = oEvent.getParameters().sortItem.getKey();
            let descending = oEvent.getParameters().sortDescending;
            ...
            items.getBinding("items").sort(oSorter);
        },
        ...
        openFilterDialog: function () {
            if (!this.fDialog) {
                this.fDialog = this.loadFragment({
                    name: "project3.view.fragments.Filter"
                });
            }

            this.fDialog.then(function (fDialog) {
                fDialog.open();
            });
        },

        handleConfirmFilter: function (oEvent) {
            let filters = oEvent.getParameters().filterKeys
            this._oListFilterState.aFilter = [];

            for (let filter in filters) {
                this._oListFilterState.aFilter.push(filter)
            }
        },
